...
A feature-rich-ish TUI music player built on top of [[https://github.com/binimum/hifi-api][TIDAL HiFi API]]: browse, search, stream, mix, batch download, find artists, organize and manage lossless music from your comfy terminal.
...
- Customizable (colors, optional TSV mode, show/hide metadata fields, file hierarchy for downloads, autoextend buffer, autoresume playback on launch, /etc./)
...
=settings.jsonc= can be edited to change UI options, colours, metadata field widths, autoextend buffer size, download destinations and naming conventions, TIDAL HiFi API URL, /etc/. Check the file for options not documented in this README.
