...
Settings are stored in =settings.json= and automatically updated upon using toggles within the TUI. Settings are resolved from:
...
Upon first execution, =tuifi= will generate these files in its directory; it is advised to move them to =~/.config/tuifi/= to conform to XDG standards.
