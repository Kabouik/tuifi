...
#+BEGIN_QUOTE
*WARNING:* Tidal has begun blocking HiFi API accounts en masse starting from around now. Until a solution is found, you may have trouble using =tuifi=, with a public or private instance. There is nothing =tuifi= can do about and issues you may encounter related to this do not come from =tuifi=, but developers are working on the HiFi side to work around the situation.
#+END_QUOTE
