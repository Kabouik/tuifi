...
class App:
    ...
    def play_track(self, t: Track, resume: bool = False, start_pos: float = 0.0) -> None:
        ...
        self._current_track_seen_tp = False
        ...
        for qi in range(start_qi, len(QUALITY_ORDER)):
            ...
            try:
                ...
                with self._play_lock:
                    ...
                    _replace_ok = False
                    _t_start = time.time()
                    if self.mp.alive() and not resume and start_pos == 0.0 and not is_local_mpd:
    ...
    def play_queue_index(self, idx: int, start_pos: float = 0.0) -> None:
        ...
        if (self.mp.alive() and _pp is not None and
                start_pos == 0.0 and
    ...
    def _gapless_invalidate_ahead(self) -> None:
        ...
        """
        if not (self.mp.alive() and self._mpv_last_pp is not None):
            return
    ...
    def show_help_dialog(self) -> None:
        ...
        lines = [
            ...
            " 0         show album cover art (req. chafa, ueberzugpp, or kitty)",
            ...
            "\x01 VIEW",
            " q         miniqueue overlay",
            " c         sideview pane: cover, spectrum (req. cava), both, off (spectrum in tab 0 only with kitty)",
            ...
            " |         desktop notifications on track change (req. libnotify)",
        ...
        if self.tab == TAB_PLAYBACK and self._cover_lyrics:
            ...
            if queue_panel:
                ...
                if _lyr_w >= 20:
                    ...
                    _cover_h = _cic // 2
                    _lyr_y = top_h + _cover_h
                    _lyr_h = usable_h - _cover_h
                    if _lyr_h >= 3:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if self.mp.alive() and self._mpv_last_pp is not None:
                _pp = self.mp.playlist_pos
                if _pp is not None and _pp > self._mpv_last_pp:
                    _prev_pp = self._mpv_last_pp
                    self._mpv_last_pp = _pp
            ...
            # started with --idle=yes (i.e., all processes now that gapless is always on).
