...
class App:
    ...
    def __init__(self, stdscr: "curses._CursesWindow", api_base: str, args: Dict[str, Any]) -> None:
        ...
        # Gapless playback: one mpv process kept alive; tracks queued via IPC playlist.
        # _mpv_qi_map[playlist-pos] → tuifi queue index for each entry in mpv's playlist.
        # _mpv_last_pp: last observed playlist-pos; None = detection disabled (mid-transition).
        self.gapless_on: bool = bool(self.settings.get("gapless_on", False))
        self._mpv_qi_map: List[int] = []
    ...
    def prompt_yes_no(self, title: str) -> bool:
        ...
        while True:
            ...
            if ch in (ord("n"), ord("N"), 27):
                break
        # Erase the popup so callers don't need to trigger their own redraw.
        self._full_redraw()
        return result
    ...
    def _resolve_stream_url_for_quality(self, track_id: int, quality: str) -> str:
        ...
        if "dash" in mime.lower() or raw_text.lstrip().startswith("<?xml") or raw_text.lstrip().startswith("<MPD"):
            ...
            # Only LOSSLESS and HI_RES_LOSSLESS should play via a local DASH .mpd file.
            # For HIGH/LOW the DASH segments use short-lived CDN auth that mpv cannot
            # re-authenticate, so playing via local .mpd always fails.  Raise instead.
            if quality not in self._MANIFESTS_FORMATS:
                debug_log(f"  DASH manifest for non-lossless quality {quality!r}: no direct URL, refusing local .mpd")
                raise RuntimeError(f"DASH manifest not playable for quality {quality!r}")
    ...
    def play_track(self, t: Track, resume: bool = False, start_pos: float = 0.0) -> None:
        ...
        # Cancel any in-flight gapless state — the new track takes over.
        self._mpv_qi_map = []
            ...
            try:
                ...
                with self._play_lock:
                    ...
                    if self.gapless_on and self.mp.alive() and not resume and start_pos == 0.0 and not is_local_mpd:
                        ...
                        debug_log(f"play_track: [TIMING] mp.replace() ok={_replace_ok} for {t.id} took {(time.time()-_t_start)*1000:.0f}ms")
                    if _replace_ok:
                        _used_replace = True
                    else:
                ...
                if self.gapless_on:
                    ...
                    self._mpv_last_pp = 0
                self._prefetch_trigger_id = None  # allow prefetch to fire for this track
                self._full_redraw()
    ...
    def toggle_priority(self, queue_idx: int) -> None:
        ...
        # The next track may have changed — clear any preloaded entries so the
        # correct track gets appended on the next prefetch trigger.
    ...
    def play_queue_index(self, idx: int, start_pos: float = 0.0) -> None:
        ...
        # Gapless shortcut: if the target track is already the next entry in mpv's
        # playlist, use playlist-next for an instant, truly gapless skip rather than
        # killing and restarting mpv (or even replace()).
        _pp = self._mpv_last_pp
        if (self.gapless_on and self.mp.alive() and _pp is not None and
                start_pos == 0.0 and
    ...
    def _toggle_gapless(self) -> None:
        ...
        self.settings["gapless_on"] = self.gapless_on
    ...
    def _on_gapless_advance(self, t: "Track", queue_idx: int) -> None:
        ...
        restarting mpv — the gapless decoder handoff already happened.
        """
        self._play_serial += 1
        ...
        # Consume prefetch metadata so quality-display fields are correct.
        _pf = self._prefetch_next
        ...
        # Update preview_next cursor — track is already playing, so timing is correct.
        if (self._preview_next and self.tab == TAB_PLAYBACK
                and self.queue_overlay and self._album_cover_pane):
            _nxt = self._preview_next_idx()
            if 0 <= _nxt < len(self.queue_items):
    ...
    def _prefetch_and_gapless_append(self, t: "Track", start_qi: int,
        ...
        """Background worker: resolve URL for *t* then, if gapless is on and the
        URL is a remote stream (not a local .mpd), append it to mpv's playlist.
    ...
    def _gapless_invalidate_ahead(self) -> None:
        ...
        Clears mpv's preloaded-ahead playlist entries and resets the qi-map so
        the next prefetch trigger will append the correct (possibly changed) track.
        """
        if not (self.gapless_on and self.mp.alive() and self._mpv_last_pp is not None):
            return
        self.mp.playlist_clear()
    ...
    def _enqueue_tracks(self, tracks: List[Track], insert_after_playing: bool) -> None:
        ...
        if not insert_after_playing:
            ...
            if self.queue_items and 0 <= self.queue_play_idx < len(self.queue_items):
                ...
                self.toast(f"Enqueued+ {len(tracks)}")
                # Inserting after current changes what comes next in the playlist.
                self._gapless_invalidate_ahead()
            else:
                self.queue_items.extend(tracks)
                if len(self.queue_items) == len(tracks):
    ...
    def _draw_playback_lyrics_panel(self, y: int, x: int, h: int, w: int) -> None:
        ...
        for row in range(inner_h):
            ...
            if y + 1 + row >= y + h:
                break
            raw = lines[li].rstrip() if li < len(lines) else ""
            # Strip control chars (nulls, escapes, carriage returns, etc.) that
            # curses cannot render and that overflow into adjacent panes.
            raw = "".join(c if c >= " " else " " for c in raw)
            safe = _truncate_to_display_width(raw, w)
            pad = w - _str_display_width(safe)
            if li == current_hit:
                ...
                attr = lyr_attr
            try:
                self.stdscr.addstr(y + 1 + row, x, safe + " " * pad, attr)
            except curses.error:
                pass
    ...
    def _draw_status(self, y: int, x: int, w: int) -> None:
        ...
        if self.show_toggles:
            ...
            if mu if mu is not None else self.desired_mute:
                parts.append("muted")
            if self.priority_queue:
                parts.append(f"priority: {len(self.priority_queue)}")
            if self._show_singles_eps:
                parts.append("ep/s")
            if self._notify_on_track:
                parts.append("notify")
            if self.gapless_on:
                parts.append("gapless")
    ...
    def show_help_dialog(self) -> None:
        ...
        lines = [
            ...
            "",
            "\x01 ACTIONS",
            " :/!/RMB   contextual menu with actions",
            ...
            "\x01 VIEW",
            " q         miniqueue overlay",
            " c         sideview pane: cover, spectrum, both, off (spectrum in tab 0 only with kitty)",
            " n         sideview cover: show next track in queue in tab 0 (req. miniqueue + minicover)",
            " Tab       move cursor between main view and miniqueue",
            ...
            " _         gapless playback (keeps mpv alive; DASH/lossless tracks only)",
            ...
            " gapless_on: keep mpv alive between tracks for gapless transitions (DASH/lossless only; default: false, toggle with _)",
    ...
    def draw(self) -> None:
        ...
        if self.tab == TAB_PLAYBACK and self._cover_lyrics:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # Track-end detection: mpv died normally (non-gapless) or crashed (gapless fallback).
            # No snapshot check needed — if alive() is False and serials match the track ended.
            if self.current_track and not self.mp.alive() and self._play_serial == self._current_track_serial:
                debug_log(f"track-end: mpv died, calling next_track() t={now:.3f}")
                self.current_track = None
                self._last_stream_quality = None
                self._mpv_qi_map = []
                self._mpv_last_pp = None
                ...
                if _playing_5s or _near_end or _buffered_enough:
                    ...
                    if _nxt_t and self._prefetch_trigger_id != _nxt_t.id:
                        ...
                        if self.gapless_on:
                            ...
                            self._bg(lambda _t=_nxt_t, _qi=_qi:
                                     self._prefetch_next_url(_t, _qi))
            ...
            # increments; we look up the tuifi queue index from _mpv_qi_map and call
            # _on_gapless_advance() to do all the bookkeeping (history, cover, etc.).
            if self.gapless_on and self.mp.alive() and self._mpv_last_pp is not None:
            ...
            # Idle detection: mpv alive but idle (track ended with --idle=yes).
            # Fires for gapless mode AND when gapless was toggled off but the
            # --idle=yes process is still running (gapless_on check removed).
            # Gated on _mpv_last_pp is not None so it only triggers for processes
            # started with gapless=True (i.e., --idle=yes).
            if self.current_track and self.mp.alive() and self._mpv_last_pp is not None:
                _tp3, _du3, *_ = self.mp.snapshot()
                # Only count ahead entries as "has content" when gapless is still on.
                _has_ahead = self.gapless_on and len(self._mpv_qi_map) > (self._mpv_last_pp or 0) + 1
                if _tp3 is None and _du3 is None and not _has_ahead and not self._prefetch_in_progress:
                    if self._gapless_idle_since is None:
                        ...
                        debug_log(f"mpv idle >0.5 s, calling next_track() t={now:.3f}")
                        self._gapless_idle_since = None
                        self.current_track = None
                        self._last_stream_quality = None
                        self.next_track()
            ...
            if self._skip_delta != 0 and now - self._skip_at >= 0.15:
                ...
                if n_q > 0:
                    ...
                    # Point cursor at the current track immediately so the miniqueue
                    # shows where we are.  The preview_next cursor (next-to-play cover)
                    # is updated only after playback confirms, in play_track() success
                    # or _on_gapless_advance(), to avoid showing the wrong cover early.
            ...
            # V in playback tab: cycle through 4 views.
            if ch == ord("V") and self.tab == TAB_PLAYBACK:
                # Cycle: lyrics → lyrics+miniqueue → miniqueue-only → cover-only → lyrics
                if self._cover_lyrics and not self.queue_overlay:
                    # lyrics → lyrics+miniqueue
                    self.queue_overlay = True
                    self.focus = "queue"
